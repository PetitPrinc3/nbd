...
- [Suggested Hardening Roadmap](#suggested-hardening-roadmap)
...
Until TLS/SASL configuration is added to `[kafka]` (see [Suggested Hardening Roadmap](#suggested-hardening-roadmap)), treat the NBD-to-broker link as **at least as sensitive as the multicast feed itself**, and mitigate at the network layer:
...
- Nothing in the current logging paths prints secret material, because there isn't any yet (e.g. `info!("Connecting to the Kafka broker at {} ...", config.kafka.broker)` only ever logs a host:port). Keep that invariant in mind when SASL credentials are eventually added — audit every `info!`/`debug!` call touching `ProducerConfig` before merging.
...
- Panics are routed to `tracing::error!` via the custom panic hook in `main.rs`. Make sure your log pipeline treats panic-derived error logs as pageable alerts, not noise — see the arithmetic-underflow finding under `LANG-ARITH` in the matrix below for a concrete scenario where that distinction matters.
...
Of the 60 rules and recommendations in the current ANSSI Secure Rust Guide checklist: **12 are fully compliant, 9 are flagged below, and 39 don't apply** — mostly the unsafe-memory and FFI rule families, which are moot as long as NBD contains no `unsafe` code and defines no foreign-function boundary of its own. The 9 flagged rows trace back to essentially three root causes, unpacked in [Known Limitations](#known-limitations).
...
| `DENV-CARGO-OPTS`    | Keep default values for critical cargo profile variables | ✅      | `[profile.release] overflow-checks = true` deviates from Rust's release default (`false`) — but in the _hardening_ direction, not the weakening one the rule guards against. See `LANG-ARITH` for the resulting trade-off. |
...
| `DENV-FORMAT`        | Use `rustfmt`                                            | ✅      | Enforced as a gate in `audit.yml` (`cargo fmt -- --check`). `build.yml` also runs plain `cargo fmt` (no `--check`), which reformats rather than gates — harmless, but it isn't the enforcement mechanism.                  |
...
| `LIBS-OUTDATED`           | Check for outdated dependencies  | ✅      | `cargo outdated` runs in `audit.yml`.                                                                                                                                    |
...
| Rule         | Requirement                      | Status | Evidence                 |
| ------------ | -------------------------------- | ------ | ------------------------ |
| `LANG-ARITH` | Guard against overflow/underflow | ✅      | `overflow-checks = true` |
...
| `LANG-LIMIT-PANIC`     | Limit `panic!` use                                               | ✅      | No direct `panic!()`/`unreachable!()`/`todo!()` in library code. `.unwrap()` does appear in `src/sinks.rs` but only for testing traits.                                                                                                                                                                                                                                                                     |
| `LANG-LIMIT-PANIC-SRC` | Limit panic-capable functions (`unwrap`, `expect`, raw indexing) | ✅      | Same as above.                                                                                                                                                                                                                                                                                                                                                                                              |
| `LANG-ARRINDEXING`     | Validate indexing or use `.get()`                                | ✅      | `IPerfSink::submit` and the `metrics-exporter` branch both validate `payload.len()` before slicing. `BenchSink::submit` does not — it calls `payload[..8]` with no preceding length check, so a UDP datagram under 8 bytes reaching a `BenchSink`-backed listener panics on an out-of-bounds slice. `BenchSink` is only reachable from test/bench binaries today, not the production `FutureProducer` path. |
...
| `LANG-UNSAFE`      | Don't use `unsafe` blocks   | ✅      | `#![forbid(unsafe_code)]` is declared at the top of `src/main.rs` and `src/lib.rs` |
...
| Rule                                                                                                             | Requirement                                                       | Status | Evidence                                                                                                                                                                                |
| ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MEM-NO-LEAK`                                                                                                    | No memory leaks                                                   | ✅      | No leaking API is currently called, but the lint that would catch the most common cause (`mem::forget`) isn't active for the crate where it would matter most — see `MEM-FORGET` below. |
| `MEM-FORGET`                                                                                                     | Do not use `mem::forget`                                          | ✅      | `#![deny(clippy::mem_forget)]` is declared in `src/main.rs` and `src/lib.rs`.                                                                                                           |
| `MEM-FORGET-LINT`                                                                                                | Use the clippy lint for `mem::forget`                             | ✅      | Same as above.                                                                                                                                                                          |
...
- **No TLS/SASL configuration surface for the Kafka connection** (see [Kafka Broker Connectivity](#kafka-broker-connectivity)) — the most significant _deployment_ gap, as distinct from a _code compliance_ gap.
...
2. Guard the `metrics-exporter` latency computation with `checked_sub`/`saturating_sub` and skip metric recording on failure instead of panicking; either document the iperf-header assumption clearly as a benchmarking-only feature flag, or make the header format explicit and opt-in per provider.
3. Bind the Prometheus exporter to loopback by default, requiring an explicit opt-in to listen on a routable interface.