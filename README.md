...
> [!IMPORTANT] AI
> While this project was not vibe-coded, its documentation was generated by Claude Opus 4.6 (Thinking).
...
│  │  (UDP Mcast) │  │  (UDP Mcast) │       │  (UDP Mcast) │  │
│  │  239.x.x.1   │  │  239.x.x.2   │       │  239.x.x.N   │  │
│  └──────┬───────┘  └──────┬───────┘       └──────┬───────┘  │
│         │                 │                      │          │
│         │  Zero-copy      │  bytes::BytesMut     │          │
│         │  split_to()     │  .freeze()           │          │
│         ▼                 ▼                      ▼          │
│  ┌───────────────────────────────────────────────────────┐  │
│  │              MessageSink Trait (Strategy)             │  │
│  │                                                       │  │
│  │  ┌──────────────────┐  ┌───────────┐  ┌───────────┐   │  │
│  │  │  FutureProducer  │  │ BenchSink │  │ IPerfSink │   │  │
│  │  │  (Kafka, prod)   │  │ (testing) │  │ (testing) │   │  │
│  │  └────────┬─────────┘  └───────────┘  └───────────┘   │  │
│  └───────────┼───────────────────────────────────────────┘  │
│              │                                              │
│              ▼                                              │
│     ┌─────────────────┐        ┌───────────────────────┐    │
│     │  Kafka Broker   │        │  Prometheus Exporter  │    │
│     │  (librdkafka)   │        │  (optional, HTTP)     │    │
│     └─────────────────┘        └───────────────────────┘    │
└─────────────────────────────────────────────────────────────┘
```
...
A comprehensive performance review is provided under [performance.md](wiki/performance.md)
...
NBD tracks compliance against the [ANSSI Secure Rust Development Guide](https://anssi-fr.github.io/rust-guide/). A detailed compliance matrix is maintained in [`wiki/ANSSI_checklist.md`](wiki/ANSSI_checklist.md).
